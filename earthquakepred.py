...
data = data[(data['Datetime'].dt.year >= 2020) & (data['Datetime'].dt.year <= 2021)]
...
plt.figure(figsize=(10, 5))
plt.scatter(y_test, y_pred, color='blue', alpha=0.5, edgecolors='k')
plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', linewidth=2)  # Ideal prediction line
plt.xlabel("Actual Magnitude")
plt.ylabel("Predicted Magnitude")
plt.title("Actual vs Predicted Magnitude (SVM) - Scaled Data")
plt.grid()
plt.show()
...
plt.xlabel("Residual")
plt.ylabel("Frequency")
plt.title("Residuals of Predicted Magnitude (SVM) - Scaled Data")
plt.grid()
plt.show()

# 3. Plot: Box Plot for Actual vs Predicted Magnitudes
plt.figure(figsize=(8, 6))
plt.boxplot([y_test, y_pred], labels=['Actual Magnitude', 'Predicted Magnitude'], patch_artist=True,
            boxprops=dict(facecolor='skyblue'), medianprops=dict(color='darkblue'))
plt.ylabel("Magnitude")
plt.title("Box Plot of Actual vs Predicted Magnitude (SVM Model - Scaled Data)")
plt.grid()
plt.show()
